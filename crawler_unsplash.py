...
def crawler_results(web, category, keyword, time_sleep):
    ...
    search_resp_results = get_search_resp_results(keyword=keyword, page=1)
    pages = search_resp_results['total_pages']
    ...
    for page in range(1, pages+1):
        ...
        if results:
            for search_info in results:   
                dl_url = search_info['urls']['raw']
                dl_url = re.sub(r'\?.*', '', dl_url)
...
def get_search_resp_results(keyword, page):
    access_key = 'fMOzyS8Tmpz5cKsusrU624uaSmpCiX5B-3AIn9XOlIc'
    search_url = f'https://api.unsplash.com/search/photos/?client_id={access_key}&query={keyword}&page={page}&per_page=30' # Max per_page 30
